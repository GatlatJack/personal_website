...
export const jobs: Project[] = [
  {
    ...
    description: '',
    bullets: [
      'An enzyme-discovery platform. A user submits a starting molecule and a target; it returns enzymes that could catalyze the transformation, drawn from published reaction databases where one is documented and generated by protein-design models where none is.',
      'Set up and maintain the production server: an Ubuntu machine in the lab running three systemd services, with a Cloudflare named tunnel serving the public site through a hospital network that blocks inbound connections.',
      'Fixed production failures including SQLite lock contention under concurrent load, a reference-data import that had never been run on the server, and molecules compared against the database in the wrong charge state.',
      'Added a mass-balance check to the discovery engine that rejects matches whose enzyme class does not agree with the atoms gained or lost.',
      'Designed the interface and built the design system — tokens and shared components — then rebuilt the submission screen, results view, and internal tools on it. Added written labels to colour-coded tags for accessibility.',
      'Built the interface for the generative design track, its biosecurity screening, and a review flow requiring a second researcher to approve a job.',
      'Set up GitHub Actions CI and wrote the deployment runbook.',
    ],
